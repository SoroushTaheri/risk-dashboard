...
const titles: Record<ViewKey, { eyebrow: [string, string]; title: [string, string]; description: [string, string] }> = {
  ...
  individual: { eyebrow: ["Modern Actuarial Risk Theory · Chapter 2", "Modern Actuarial Risk Theory · فصل ۲"], title: ["From policy risks to aggregate loss", "از ریسک هر بیمه‌نامه تا خسارت کل"], description: ["Each selected policy keeps its full monthly paid-loss distribution. Chapter 2 approximations are compared with the same-month empirical aggregate to isolate the effect of cross-policy dependence.", "توزیع کامل خسارت پرداختی ماهانه‌ی هر بیمه‌نامه‌ی انتخاب‌شده حفظ می‌شود. تقریب‌های فصل ۲ با مجموع تجربیِ هم‌ماه مقایسه می‌شوند تا اثر وابستگی میان بیمه‌نامه‌ها جدا و سنجیده شود."] },
      ...
      <main>
        <header className="topbar">
          ...
          <div className="topbar-actions">
            <div className="course-chip">{tr(language, "SBU · Semester 2 · 1404–1405", "پروژه پایانی درس نظریه ریسک · دانشگاه شهید بهشتی · نیم‌سال دوم ۱۴۰۴–۱۴۰۵")}</div>
        ...
        <footer className="course-footer" aria-label={tr(language, "Course context", "زمینه‌ی درس")}>
          {tr(
            language,
            "Final project for Risk Theory in the M.Sc. in Actuarial Sciences program at Shahid Beheshti University · Second semester 1404–1405",
            "پروژه‌ی پایانی درس نظریه ریسک · دوره‌ی کارشناسی ارشد علوم بیم‌سنجی دانشگاه شهید بهشتی · نیم‌سال دوم ۱۴۰۴–۱۴۰۵",
          )}
        </footer>
      </main>
    </div>
...
function Experience({ view, data }: { view: ViewKey; data: NonNullable<ReturnType<typeof usePortfolio>["data"]> }) {
  ...
  return <RuinExperience data={data} />;
}
