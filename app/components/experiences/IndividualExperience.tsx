...
export function IndividualExperience({ data }: { data: PortfolioData }) {
  ...
  return (
    <div className="experience-stack">
      <ReferenceBand source={t("Modern Actuarial Risk Theory · Chapter 2 · Individual risk model", "نظریهٔ مدرن ریسک بیمه‌ای · فصل ۲ · مدل ریسک انفرادی")} formula="S = ΣᵢXᵢ, E[S] = Σᵢqᵢbᵢ, Fₓ₊ᵧ = Fₓ * Fᵧ">
        {t("Each synthetic policy contributes a loss variable Xᵢ with probability qᵢ and benefit proxy bᵢ. Their sum S can be built by convolution only under the stated independence assumption; the other scenarios show the effect of dependence.", "هر بیمه‌نامهٔ مصنوعی یک متغیر خسارت Xᵢ با احتمال qᵢ و مبلغ تعهد تقریبی bᵢ دارد. مجموع S فقط تحت فرض استقلال اعلام‌شده با پیچش ساخته می‌شود؛ سناریوهای دیگر اثر وابستگی را نشان می‌دهند.")}
      </ReferenceBand>
      ...
      <section className="control-strip" aria-label={t("Individual risk controls", "کنترل‌های مدل ریسک انفرادی")}>
        ...
        <label><span>{t("Policy set", "گروه بیمه‌نامه")}</span><select value={segment} onChange={(event) => setSegment(event.target.value)}><option value="all">{t("All synthetic policies", "همهٔ بیمه‌نامه‌های مصنوعی")}</option><option value="preferred">{t("Preferred", "کم‌ریسک")}</option><option value="standard">{t("Standard", "استاندارد")}</option><option value="commercial">{t("Commercial", "تجاری")}</option></select></label>
        <label><span>{t("Dependence assumption", "فرض وابستگی")}</span><select value={scenario} onChange={(event) => setScenario(event.target.value)}><option value="independent">{t("Independent Xᵢ", "Xᵢ مستقل")}</option><option value="shared">{t("Shared accident exposure", "مواجههٔ مشترک با حادثه")}</option><option value="linked">{t("Policy-linked reconstruction", "بازسازی پیوندخورده با بیمه‌نامه")}</option></select></label>
      ...
      <div className="metric-grid four">
        <MetricCard label="E[S] = Σqᵢbᵢ" value={money(expectation)} detail={t("Reconstructed policy inputs", "ورودی‌های بازسازی‌شدهٔ بیمه‌نامه")} tone="blue" />
