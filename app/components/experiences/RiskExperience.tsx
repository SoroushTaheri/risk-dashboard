...
export function RiskExperience({ data }: { data: PortfolioData }) {
  ...
  return (
    <div className="experience-stack">
      ...
      <Contributor names={contributors} summary={t("The submitted empirical, Normal, and EVT-GPD VaR/TVaR methods are applied to the same 1,000 monthly aggregate insurer-paid losses, with consistent tail conventions, EVT threshold checks, and X_R=min(X,d) for retained loss. Delta–Gamma is retained only as a hypothetical example separate from the portfolio data—not a fourth estimator—and the autoregressive route is omitted because the synthetic months are not a chronological time series.", "روش‌های ارائه‌شده‌ی VaR و TVaR در حالت‌های تجربی، نرمال و EVT-GPD بر همان ۱٬۰۰۰ مشاهده‌ی خسارت تجمیعی ماهانه‌ی پرداخت‌شده توسط بیمه‌گر اعمال شده‌اند؛ با تعریف یکسان دنباله، کنترل آستانه‌ی EVT و رابطه‌ی X_R=min(X,d) برای خسارت نگهداری‌شده. دلتا–گاما فقط به‌صورت مثالی فرضی و جدا از داده‌های پرتفوی آورده شده است، نه به‌عنوان برآوردگر چهارم؛ روش خودرگرسیونی نیز چون ماه‌های مصنوعی یک سری زمانی تقویمی نیستند، استفاده نشده است.")} />
    </div>
  );
}
