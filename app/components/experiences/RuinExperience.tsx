...
export function RuinExperience({ data }: { data: PortfolioData }) {
  ...
  return (
    <div className="experience-stack">
      <ReferenceBand source={t("Modern Actuarial Risk Theory · Chapter 4 · Ruin theory", "نظریهٔ مدرن ریسک بیمه‌ای · فصل ۴ · نظریهٔ ورشکستگی")} formula="Uₙ=u+Σᵢ₌₁ⁿ(c−Sᵢ), c=(1+θ)E[S], ψ(u)≤e⁻ᴿᵘ">
        {t("Initial capital u, safety loading θ, retained aggregate loss Sᵢ, and horizon n determine the simulated surplus path. The Lundberg bound is displayed only when the severity model admits a positive adjustment coefficient R.", "سرمایهٔ اولیه u، ضریب اطمینان θ، خسارت کل نگهداری‌شده Sᵢ و افق n مسیر شبیه‌سازی‌شدهٔ مازاد را تعیین می‌کنند. کران لوندبرگ فقط زمانی نمایش داده می‌شود که مدل شدت ضریب تعدیل مثبت R داشته باشد.")}
      </ReferenceBand>

      <section className="control-strip" aria-label={t("Solvency controls", "کنترل‌های توانگری")}>
        <label className="range-control wide"><span>{t("Initial capital (u)", "سرمایهٔ اولیه (u)")} <strong>{money(capital)}</strong></span><input type="range" min="0" max={data.summary.mean_payout * 10} step={500000} value={capital} onChange={(event) => setCapital(Number(event.target.value))} /></label>
      ...
      <div className="panel-grid two-thirds">
        ...
        <section className="panel">
          <div className="panel-heading"><div><ResultTag tone="slate">{t("Capital sensitivity", "حساسیت به سرمایه")}</ResultTag><h2>{t("ψₙ(u) as initial capital u changes", "تغییر ψₙ(u) با سرمایهٔ اولیه u")}</h2><p>{t("The chapter predicts a non-increasing ruin probability as u rises; this controlled curve makes that property visible.", "فصل پیش‌بینی می‌کند با افزایش u احتمال ورشکستگی افزایش نیابد؛ این منحنی کنترل‌شده همان ویژگی را نشان می‌دهد.")}</p></div></div>
          <EChart height={275} label={t("Finite-horizon ruin probability by initial capital", "احتمال ورشکستگی در افق محدود بر حسب سرمایهٔ اولیه")} option={{ animation: false, grid: { left: 58, right: 16, top: 16, bottom: 48 }, tooltip: { trigger: "axis", valueFormatter: (value: unknown) => `${(Number(value) * 100).toFixed(1)}%` }, xAxis: { type: "category", data: capitalCurve.map((row) => `${Math.round(row[0] / 1e6)}m`), axisLabel: { interval: 2 }, name: t("initial capital u", "سرمایهٔ اولیه u"), nameLocation: "middle", nameGap: 30 }, yAxis: { type: "value", min: 0, max: 1, axisLabel: { formatter: (value: number) => `${Math.round(value * 100)}%` }, splitLine: { lineStyle: { color: "#e8edf3" } } }, series: [{ type: "line", smooth: true, symbolSize: 5, data: capitalCurve.map((row) => row[1]), lineStyle: { color: "#c63f49", width: 2.3 }, itemStyle: { color: "#c63f49" }, areaStyle: { color: "rgba(198,63,73,.09)" } }] }} />
          <PanelCredit names={contributors} />
        </section>
      </div>

      <div className="formula-grid"><Formula equation={String.raw`U_n=u+\sum_{i=1}^{n}(c-S_i)`} label={t("Discrete surplus equals initial capital plus accumulated premium margin after aggregate losses.", "مازاد گسسته برابر سرمایهٔ اولیه به‌علاوهٔ حاشیهٔ حق‌بیمهٔ انباشته پس از خسارت‌های کل است.")} hint={t("Ruin", "ورشکستگی")} /><Formula equation={String.raw`c=(1+\theta)E[S]`} label={t("Premium per period applies safety loading θ to expected retained aggregate loss.", "حق‌بیمهٔ هر دوره ضریب اطمینان θ را بر امید خسارت کل نگهداری‌شده اعمال می‌کند.")} hint={t("Safety loading", "ضریب اطمینان")} /><Formula equation={String.raw`\psi(u)\le e^{-Ru}`} label={t("The Lundberg upper bound requires a valid positive adjustment coefficient R.", "کران بالای لوندبرگ به ضریب تعدیل مثبت و معتبر R نیاز دارد.")} /></div>
      {!theoreticalAvailable ? <Notice kind="warning" title={t("Lundberg theory is disabled for the raw Pareto tail", "نظریهٔ لوندبرگ برای دنبالهٔ پارتوی خام غیرفعال است")}>{t("Finite-horizon simulation remains available, but an unbounded Pareto severity has no positive MGF. No adjustment coefficient is invented and the result is not relabeled as ultimate ruin.", "شبیه‌سازی افق محدود همچنان معتبر است، اما شدت پارتوی نامحدود تابع مولد گشتاور مثبت ندارد. ضریب تعدیل ساختگی تولید نمی‌شود و نتیجه نیز ورشکستگی نهایی نام نمی‌گیرد.")}</Notice> : <Notice kind="info" title={t("Finite-horizon estimate and ultimate bound are different quantities", "برآورد افق محدود و کران نهایی دو کمیت متفاوت‌اند")}>{t("The red estimate is finite-horizon Monte Carlo. The Lundberg value is an upper bound for ultimate ruin under stronger light-tail assumptions; it is shown only for theoretical context.", "برآورد قرمز مونت‌کارلوی افق محدود است. مقدار لوندبرگ کران بالای ورشکستگی نهایی تحت فرض‌های قوی‌تر سبک‌دنباله است و فقط برای زمینهٔ نظری نمایش داده می‌شود.")}</Notice>}
      <Contributor names={contributors} files="insurance_ruin_analysis.py" summary={t("The submitted ruin simulation was extended with deterministic paths, finite-horizon labels, consistent retained-loss premiums, Monte Carlo uncertainty, and valid heavy-tail refusal behavior.", "شبیه‌سازی ورشکستگی ارائه‌شده با مسیرهای قطعی، برچسب افق محدود، حق‌بیمهٔ سازگار با خسارت نگهداری‌شده، عدم‌قطعیت مونت‌کارلو و رفتار صحیح در دنبالهٔ سنگین توسعه یافته است.")} />
    </div>
  );
}
