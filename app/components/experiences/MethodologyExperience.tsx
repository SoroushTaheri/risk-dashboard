...
export function MethodologyExperience({ data }: { data: PortfolioData }) {
  ...
  const resultTypes = [
    [t("Source data", "دادهٔ منبع"), t("Direct calculation from the immutable supplied CSV.", "محاسبهٔ مستقیم از CSV منبع که تغییر نمی‌کند."), "blue"],
    ...
    [t("Empirical", "تجربی"), t("A statistic calculated from the supplied or reconstructed sample.", "آماره‌ای که از نمونهٔ منبع یا بازسازی‌شده محاسبه می‌شود."), "blue"],
    ...
    [t("Simulated", "شبیه‌سازی‌شده"), t("A fixed-seed Monte Carlo calculation with sampling uncertainty.", "محاسبهٔ مونت‌کارلو با بذر ثابت و عدم‌قطعیت نمونه‌گیری."), "red"],
  ...
  const contributors = [
    { chapter: t("Source portfolio", "پرتفوی منبع"), names: "علی تیموری", files: "insurance_simulated_data.csv", work: t("Synthetic aggregate motor-insurance data", "داده‌های مصنوعی تجمیعی بیمهٔ خودرو") },
    { chapter: t("Chapter 2 - Dr. Payandeh", "فصل ۲ - دکتر پاینده"), names: "محمدرضا سعیدخانی، محمد مهدوی نسب، علی جهانبان، محمد اشکوری، نجمه زارع", files: "Risk_Measures_and_Risk_Comparison.py · دلتا گاما.R", work: t("VaR, risk comparison, and delta–gamma concepts", "VaR، مقایسهٔ ریسک و مفاهیم دلتا–گاما") },
    { chapter: t("Modern Ch. 1 · Utility", "فصل ۱ مدرن · مطلوبیت"), names: "ابوالفضل اقراری، حامد اشراقی", files: "Chapter1.ipynb.json", work: t("Utility, certainty equivalents, and acceptable premiums", "مطلوبیت، معادل قطعی و حق‌بیمهٔ قابل‌قبول") },
  ...
  return (
    <div className="experience-stack methodology">
      ...
      <div className="audit-banner">
        ...
        <div><ResultTag tone="green">{t("Reconciliation passed", "تطبیق داده‌ها تأیید شد")}</ResultTag><h2>{t("1,000 of 1,000 source months validated", "هر ۱۰۰۰ ماه منبع اعتبارسنجی شدند")}</h2><p>{t(`The largest relative reconciliation difference is ${data.summary.max_relative_difference.toExponential(2)}. Derived data are checked before any chapter calculation uses them.`, `بیشترین اختلاف نسبی تطبیق ${data.summary.max_relative_difference.toExponential(2)} است. داده‌های مشتق‌شده پیش از استفاده در محاسبات هر فصل کنترل می‌شوند.`)}</p></div>
      ...
      <section className="panel credits-panel">
        <div className="panel-heading"><div><ResultTag tone="blue">{t("Academic credit", "اعتبار علمی")}</ResultTag><h2>{t("Contributors by calculation area", "مشارکت‌کنندگان به تفکیک حوزهٔ محاسبه")}</h2><p>{t("Original submissions remain unchanged. Integrated modules preserve matching source evidence and adaptation notes.", "کارهای اصلی بدون تغییر باقی مانده‌اند. ماژول‌های یکپارچه شواهد منبع و یادداشت‌های تطبیق متناظر را حفظ می‌کنند.")}</p></div></div>
        <div className="credits-table" role="table" aria-label={t("Student contributions", "مشارکت دانشجویان")}>
          <div className="credits-head" role="row"><span>{t("Reference area", "حوزهٔ منبع")}</span><span>{t("Contributors", "مشارکت‌کنندگان")}</span><span>{t("Source files", "فایل‌های منبع")}</span><span>{t("Calculation contribution", "مشارکت محاسباتی")}</span></div>
