...
export function CollectiveExperience({ data }: { data: PortfolioData }) {
  ...
  return (
    <div className="experience-stack">
      <ReferenceBand source={t("Modern Actuarial Risk Theory · Chapter 3 · Collective risk model", "نظریهٔ مدرن ریسک بیمه‌ای · فصل ۳ · مدل ریسک جمعی")} formula="S = Σᵢ₌₁ᴺXᵢ, E[S]=E[N]E[X]">
      ...
      <section className="control-strip" aria-label={t("Collective risk controls", "کنترل‌های مدل ریسک جمعی")}>
        ...
        <label><span>{t("Method for Fₛ", "روش محاسبهٔ Fₛ")}</span><select value={method} onChange={(event) => setMethod(event.target.value)}><option value="panjer">{t("Panjer recursion", "بازگشت پانژر")}</option><option value="fft">{t("FFT inversion", "وارون‌سازی FFT")}</option><option value="simulation">{t("Monte Carlo", "مونت‌کارلو")}</option></select></label>
        ...
        <label className="range-control"><span>{t("Severity grid step (h)", "گام شبکهٔ شدت (h)")} <strong>{money(step)}</strong></span><input type="range" min="50000" max="1000000" step="50000" value={step} onChange={(event) => setStep(Number(event.target.value))} /></label>
      </section>

      <div className="metric-grid four">
        <MetricCard label="E[N]" value={countMean.toFixed(1)} detail={t("Mean monthly accident count", "میانگین تعداد حادثهٔ ماهانه")} tone="blue" />
      ...
      <div className="method-cards"><article><span>01</span><div><strong>{t("Panjer recursion", "بازگشت پانژر")}</strong><p>{t("Recursive calculation on a discrete severity grid for (a,b,0) frequency classes.", "محاسبهٔ بازگشتی روی شبکهٔ گسستهٔ شدت برای خانواده‌های فراوانی (a,b,0).")}</p></div></article><article><span>02</span><div><strong>{t("FFT inversion", "وارون‌سازی FFT")}</strong><p>{t("Fast transform inversion; grid width must control circular wrap-around error.", "وارون‌سازی سریع تبدیل؛ عرض شبکه باید خطای پیچش دوری را کنترل کند.")}</p></div></article><article><span>03</span><div><strong>{t("Monte Carlo", "مونت‌کارلو")}</strong><p>{t("Reproducible simulation with sampling error instead of only discretization error.", "شبیه‌سازی بازتولیدپذیر با خطای نمونه‌گیری، علاوه بر خطای گسسته‌سازی.")}</p></div></article></div>
      <div className="formula-grid"><Formula equation={String.raw`S=\sum_{i=1}^{N}X_i`} label={t("Aggregate loss compounds a random claim count N with severities Xᵢ.", "خسارت کل، تعداد تصادفی N را با شدت‌های Xᵢ ترکیب می‌کند.")} /><Formula equation={String.raw`E[S]=E[N]E[X]`} label={t("Expected aggregate loss factors under independence of N and identically distributed severities.", "امید خسارت کل تحت استقلال N و شدت‌های هم‌توزیع به حاصل‌ضرب دو امید تجزیه می‌شود.")} /><Formula equation={String.raw`\operatorname{Var}(S)=E[N]\operatorname{Var}(X)+\operatorname{Var}(N)E[X]^2`} label={t("Aggregate variance contains separate severity and frequency components.", "واریانس خسارت کل دو مؤلفهٔ جداگانهٔ شدت و فراوانی دارد.")} /></div>
      <Notice kind="warning" title={t("Grid step h changes the numerical result", "گام شبکهٔ h نتیجهٔ عددی را تغییر می‌دهد")}>{t("Panjer and FFT require a discretized severity distribution. A coarser h is faster but can distort the aggregate tail; represented probability mass and truncation error must be checked.", "پانژر و FFT به توزیع شدت گسسته نیاز دارند. h بزرگ‌تر سریع‌تر است اما می‌تواند دنبالهٔ خسارت کل را تحریف کند؛ جرم احتمال نمایش‌داده‌شده و خطای برش باید کنترل شوند.")}</Notice>
      <Contributor names={contributors} files="Sections 3.5–3.10" summary={t("The submitted Panjer, FFT, approximation, policy, fitting, and stop-loss work was consolidated into one normalized library with explicit assumptions and corrected sparse-vector behavior.", "کارهای ارائه‌شده دربارهٔ پانژر، FFT، تقریب، بیمه‌نامه، برازش و مازاد خسارت در یک کتابخانهٔ نرمال‌شده با فرض‌های صریح و رفتار اصلاح‌شدهٔ بردار تنک یکپارچه شده‌اند.")} />
    </div>
  );
}
