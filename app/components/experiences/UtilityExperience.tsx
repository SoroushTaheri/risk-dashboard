...
export function UtilityExperience({ data }: { data: PortfolioData }) {
  ...
  return (
    <div className="experience-stack">
      <ReferenceBand source={t("Modern Actuarial Risk Theory · Chapter 1 · Utility and insurance", "Modern Actuarial RIsk Theory · فصل ۱ · مطلوبیت و بیمه")} formula="E[u(w−X)] = u(w−P⁺), Xᴿ=min(X,d), πX(d)=E[(X−d)₊]">
        {t("The decision maker's risk aversion changes the indifference premium P⁺. Retention d splits every loss X into retained and ceded layers, and expected ceded loss is the net stop-loss premium.", "ریسک‌گریزی تصمیم‌گیرنده حق‌بیمه‌ی بی‌تفاوتی P⁺ را تغییر می‌دهد. حد نگهداری d هر خسارت X را به بخش نگهداری‌شده و واگذارشده تقسیم می‌کند و امید خسارت واگذارشده همان حق‌بیمه‌ی خالص مازاد خسارت است.")}
      </ReferenceBand>

      <section className="control-strip" aria-label={t("Utility and reinsurance controls", "کنترل‌های مطلوبیت و بیمه‌ی اتکایی")}>
        ...
        <label><span>{t("Severity distribution", "توزیع شدت خسارت")}</span><select value={model} onChange={(event) => setModel(event.target.value)}><option value="empirical">{t("Empirical sample", "نمونه‌ی تجربی")}</option><option value="lognormal">{t("Fitted lognormal", "لگ‌نرمال برازش‌شده")}</option><option value="pareto">{t("Fitted Pareto", "پارتوی برازش‌شده")}</option></select></label>
      ...
      <div className="metric-grid four">
        ...
        <MetricCard label="πX(d)" value={money(stopLoss)} detail={t("Net stop-loss premium; no loading", "حق‌بیمه‌ی خالص مازاد خسارت؛ بدون بار") } tone="amber" />
      ...
      <div className="panel-grid two-thirds">
        ...
        <section className="panel decision-panel">
          <div className="panel-heading"><div><ResultTag tone="amber">{t("Premium interval", "بازه‌ی حق‌بیمه")}</ResultTag><h2>{t("Minimum ceded loss and maximum acceptable premium", "حداقل خسارت واگذارشده و حداکثر حق‌بیمه‌ی قابل‌قبول")}</h2><p>{t("A feasible agreement requires the reinsurer's minimum premium and the insurer's utility-based maximum P⁺ to overlap.", "توافق زمانی ممکن است که حداقل حق‌بیمه‌ی بیمه‌گر اتکایی و حداکثر مبتنی بر مطلوبیت بیمه‌گر P⁺ هم‌پوشانی داشته باشند.")}</p></div></div>
          ...
          <dl className="compact-dl"><div><dt>{t("SD(Xᴿ)", "انحراف معیار Xᴿ")}</dt><dd>{money(retainedSd)}</dd></div><div><dt>{t("Ceded share of E[X]", "سهم واگذارشده از E[X]")}</dt><dd>{((stopLoss / grossMean) * 100).toFixed(1)}%</dd></div><div><dt>X = Xᴿ + Xᶜ</dt><dd>{t("Exact for every month", "برای همه‌ی ماه‌ها دقیق است")}</dd></div></dl>
          <PanelCredit names={contributors} />
        </section>
      </div>

      <div className="formula-grid">
        <Formula equation={String.raw`E[u(w-X)]=u(w-P^+)`} label={t("P⁺ leaves the decision maker indifferent between bearing X and paying for protection.", "P⁺ تصمیم‌گیرنده را میان تحمل X و پرداخت برای پوشش بی‌تفاوت می‌کند.")} hint={t("Utility · acceptable premium", "مطلوبیت · حق‌بیمه‌ی قابل‌قبول")} />
        <Formula equation={String.raw`X_R=\min(X,d),\qquad X_C=(X-d)_+`} label={t("Every loss is split exactly into retained and ceded layers at retention d.", "هر خسارت در حد نگهداری d دقیقاً به دو لایه‌ی نگهداری‌شده و واگذارشده تقسیم می‌شود.")} hint={t("Retention", "حد نگهداری")} />
        <Formula equation={String.raw`\pi_X(d)=E[(X-d)_+]`} label={t("The net stop-loss premium is expected ceded loss before expenses and risk loading.", "حق‌بیمه‌ی خالص مازاد خسارت، امید خسارت واگذارشده پیش از هزینه‌ها و بار ریسک است.")} />
      </div>
      {heavyInvalid ? <Notice kind="warning" title={t("Entropic premium is not defined", "حق‌بیمه‌ی آنتروپی تعریف نمی‌شود")}>{t("An unbounded Pareto loss has no finite positive moment-generating function. Bound the retained loss by lowering d, or explicitly use the finite empirical sample.", "خسارت پارتوی نامحدود تابع مولد گشتاور مثبت متناهی ندارد. با کاهش d خسارت نگهداری‌شده را کراندار کنید یا صریحاً از نمونه‌ی تجربی متناهی استفاده کنید.")}</Notice> : <Notice kind="info" title={t("Finite-sample calculation", "محاسبه بر پایه‌ی نمونه‌ی متناهی")}>{t("The displayed certainty-equivalent result uses the 1,000 supplied synthetic months; it is not labeled as a closed-form MGF result.", "نتیجه‌ی معادل قطعی با هزار ماه مصنوعی منبع محاسبه شده و به‌عنوان نتیجه‌ی بسته‌ی تابع مولد گشتاور برچسب نمی‌خورد.")}</Notice>}
