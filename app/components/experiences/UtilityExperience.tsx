...
export function UtilityExperience({ data }: { data: PortfolioData }) {
  ...
  const { data: apiResult, error: apiError } = useActuarialApi<UtilityApiResult>("/api/utility-reinsurance", {
  ...
  return (
    <div className="experience-stack">
      ...
      <Contributor names={contributors} files="Chapter1.ipynb.json" summary={t("The submitted exponential, logarithmic, and power utility functions were connected to one exact expected-utility premium equation, with explicit parameters, domain checks, and a shared retained-loss definition.", "توابع مطلوبیت نمایی، لگاریتمی و توانی ارائه‌شده به یک معادله دقیق حق‌بیمه بر پایه امید مطلوبیت متصل شده‌اند؛ با پارامترهای صریح، کنترل دامنه و تعریف یکسان خسارت نگهداری‌شده.")} />
    </div>
  );
}
